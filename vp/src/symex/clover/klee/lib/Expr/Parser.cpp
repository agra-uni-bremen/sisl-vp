...
namespace {
  ...
  class ParserImpl : public Parser {
    ...
    const Identifier *CreateIdentifier(std::string Name);
  ...
  public:
    ...
    virtual ExprHandle ParseSingleConstraint(std::map<std::string, ExprHandle> env);
...
const Identifier *ParserImpl::CreateIdentifier(std::string Name) {
  // Identifier with given name must not exist yet.
  assert(IdentifierTab.find(Name) == IdentifierTab.end());
...
ExprHandle ParserImpl::ParseSingleConstraint(std::map<std::string, ExprHandle> env) {
  // Shouldn't be neccessary to clear ExprSymTab since we operate on a
  // new parser anyhow. Otherwise, the IdentifierTab must also be cleared.
  //ExprSymTab.clear();

  for (auto pair : env) {
    auto id = CreateIdentifier(pair.first);
    ExprSymTab.insert(std::make_pair(id, pair.second));
  }

  ExprResult expr = ParseExpr(TypeResult(Expr::Bool));
  if (!expr.isValid()) {
